...
class Block1BannerUpdateImageController
{
    /**
    ...
    /**
     * Block1BannerUpdateImageController constructor.
     *
    ...
    public function handleAction(SalesFunnelWebinar $funnel, Request $request)
    {
        ...
        if ($fileUuid && $fileCdn && $fileName) {
            if ($funnel->getImageBanner()) {
                $this->uploadCareService->removeFile($funnel->getImageBanner()->getFileUuid());
                $this->entityManager->remove($funnel->getImageBanner());
            }
            $file = new UploadCareFile($fileUuid, $fileCdn, $fileName);

            $this->entityManager->persist($file);

            $funnel->setImageBanner($file);

            $this->entityManager->persist($funnel);

            $this->entityManager->flush();
            $this->courseAccessService->registerImageAddEvent($file, [
                'description' => 'Вебинарная воронка, блок 1, баннер',
